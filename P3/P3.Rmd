...
# Prediksi Market Value Striker di Liga Inggris Musim 25/26 Berdasarkan Jumlah Gol yang Dicetak 
...
dt <- read.csv2("Marketvalue.csv", sep=',')

dt <- dt %>%
  mutate(
    value = market.value %>%
      str_remove_all("Rp|Mlyr\\.") %>% 
      str_remove_all("\\.") %>%        
      str_replace(",", ".") %>%        
      as.numeric()
  )
...
model <- lm(value ~ sisa.bulan, data = dt)
...
plot(dt$x, dt$y, 
     pch = 19, 
     col = "gray", 
     main = "Visualisasi Regresi Market Value",
     xlab = "Jumlah Gol", 
     ylab = "Market Value (Miliar)")

abline(model, col = "red", lwd = 2)

text(max(dt$x), predict(model, data.frame(x=max(dt$x))), 
     labels = "Y_duga", pos = 3, col = "red", cex = 0.8)

abline(h = y.bar, col = "blue", lty = 3, lwd = 2)

text(max(dt$x), y.bar, 
     labels = "Y_bar", pos = 3, col = "blue", cex = 0.9)
```
...
$$JKT = JKR + JKG$$
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 6054664 dan JKG model bernilai 11765326 
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{161168.8} \approx 401.4584$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar  434.21  dan 65.81 Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [196.927 ,  671.5006] dan untuk parameter $β_1$ adalah [42.92517 ,  88.69721]
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=10 adalah 1421.382 Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang  1188.819 hingga 1653.944
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah  1421.382 Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang 588.1626 hingga 2254.601
